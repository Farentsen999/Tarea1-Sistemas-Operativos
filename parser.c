...
int detectorCantidadPipes(char *inputTokens[], char **comandos[]) {
    int pipes = 0;
    int arg_count = 0;
    static char *command_line[MAX_STRINGS][MAX_ARGS];  // static para que exista despues de que la funcion haga return
...
void commandParser(char *inputTokens[], char *args[]) {
    ...
    int pipes = detectorCantidadPipes(inputTokens, comandos);

    if (pipes != 0) {
        exec_with_pipes(comandos, pipes);
    } else {            // SIN PIPES
        int arg_count = 0;
        for (; inputTokens[arg_count] != NULL; arg_count++) {
            args[arg_count] = inputTokens[arg_count];
        }
        args[arg_count] = NULL;
    }
